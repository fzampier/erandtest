...
The hierarchy of over-betting risk is: \textbf{continuous $>$ binary $\gg$ survival}. This maps directly to how frequently the wealth product is updated. More frequent updates mean more opportunities for over-betting to compound, requiring increasingly conservative or adaptive strategies.
...
The e-RT family comprises six nonparametric sequential tests for randomized trials based on the betting framework for e-values (i-bet \cite{pmlr-v177-duan22a}). All variants require only that treatment assignment is randomized---no distributional assumptions about outcomes are needed. This makes them robust complements to model-based analyses. The variants cover binary outcomes (e-RTb), event-only monitoring (e-RTe), continuous endpoints (e-RTc), time-to-event analyses (e-RTs), multi-state trajectory data (e-RTms), and a universal abstraction (e-RTu, under development).
