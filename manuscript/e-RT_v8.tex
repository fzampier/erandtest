...
\begin{itemize}
    ...
    \item If $Y_i$ is extreme (large $|g_i|$) \emph{and} past data show a substantial treatment effect (large $|\hat{d}|$), then $\lambda_i$ deviates substantially from 0.5: we place a confident bet on one arm. The product $g_i \cdot \hat{d}_{i-1}$ determines both direction and magnitude.
    \item If $Y_i$ is extreme but $\hat{d} \approx 0$ (no estimated effect yet), $\lambda_i$ stays near 0.5---an unusual observation is not informative if we do not yet know which direction to bet.
...
The test was run sequentially with a burn-in period and ramp (burn-in = 50 patients, ramp = 100 patients, $c_{\max} = 0.6$). Under the null (no mean difference between arms), Type I error was close to or below the nominal level. Under the alternative, the implementation for continuous endpoints rejected the null with moderate power and typically crossed the threshold at an intermediate sample size (Table~\ref{tab:simulations_continuous}). Again, we believe that this may not be a replacement for the fixed-sample $t$-test but a conservative, anytime-valid monitoring tool that can trigger early stopping when effects are larger or clearer than anticipated.

\begin{table}[htbp]
\centering
\caption{Operating Characteristics for Continuous Outcomes}
\begin{tabular}{@{}cccccc@{}}
\toprule
...
\bottomrule
\end{tabular}
\label{tab:simulations_continuous}
\end{table}
...
\textbf{Continuous (e-RTc):} The problem is even worse. Beyond the dense updates (every patient), the $g$-score introduces a second source of variability in each wager. Fixed-wage betting is catastrophic at all tested levels for effects of $d \leq 0.20$.
...
\begin{table}[htbp]
\centering
\caption{Continuous: Adaptive vs.\ Fixed Direction Magnitude}
\begin{tabular}{@{}ccccc@{}}
\toprule
